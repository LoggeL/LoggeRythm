...
**Source revision:** base `e2ecf76`; current evidence is the finalized Media3 candidate tree for `android-media3-v1.0.2-rc.1` on 2026-07-16
...
| Remote release evidence | **Prepared for QA prerelease; exact remote publication pending this revision** | The clean first-party candidate targets branch/tag `android-media3-v1.0.2-rc.1` in `LoggeL/LoggeRythm`. The release must remain marked prerelease and explicitly disclose debug signing and the production compatibility 404; a production/latest release is not accepted by this evidence. |
...
Current first-party evidence includes offline `npm ci`, TypeScript, ESLint, 149 Vitest files / 1,003 tests plus one todo, 59/59 JVM tests, 5/5 API 36 instrumentation tests, clean Expo prebuild, release lint/assembly/R8, a 426-source/1,090-APK zero-finding gate, and fresh-install cold/warm startup on the Android Studio emulator. Historical v10005 production playback/queue/like evidence and b10012 auth-layout evidence remain regression references only; b10012 contains RNTP/derivative code and must not be published. QA-04 stays open because controller trust still needs real hostile cross-UID instrumentation and remote CI. QA-06 remains open for authenticated atomic logout/account-switch cleanup. QA-07/08 remain partial because production's compatibility 404 prevents current-source authenticated playback and lifecycle/repeat/radio/background/Auto/reboot matrices are incomplete. QA-18 remains partial until exact remote commit/workflow/backend revision and the explicit manual-exceptions manifest are recorded.
