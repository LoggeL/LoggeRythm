...
**Acceptance snapshot:** 28/62 P0 and 77/124 P1 work items are closed at this evidence revision; 34 P0 and 47 P1 items remain open.

**Baseline:** 2026-07-15; evidence updated through 2026-07-16

**Source revision:** base `e2ecf76`; current evidence is from the uncommitted working tree on 2026-07-16

**Production reference:** `https://loggerythm.logge.top`

**Android reference:** production-connected `1.0.1` (`versionCode 10005`) remains the last APK with authenticated production playback evidence. The exact current working tree is assembled and installed on Android API 36 as optimized ARM64 `1.0.1`/`10012` QA/debug-signed. Production's HTTP 404 at `/api/version` correctly blocks authentication/API/media access. The local **Forget session and sign in** cleanup defect first reproduced in v10009 remains fixed, and v10012 also closes the specific short-landscape auth clipping defect found in v10011. Account-switch/filesystem/notification/Android Auto cleanup and the broader responsive/device matrices remain release gates.
...
- Keep RNTP/Media3 as the single playback authority. Do not create a second JavaScript-only playback state.
...
The largest technical seam is not React Navigation; it is the 3,870-line patch to commercially licensed `@rntp/player` in [`mobile/patches/@rntp+player+5.6.0.patch`](../mobile/patches/@rntp+player+5.6.0.patch). It touches controller lifecycle, authenticated headers, encryption, persistence, cache eviction, sleep-timer completion, and car browsing. A clean `npm ci --offline` applies the patch without drift, and its current SHA-256 is `d0a6bd799aef1711a4f8555f9d8941ed745750c6c9ed1ade21623d511f92ece3`. The repository has no recorded written permission covering public APK/AAB distribution of this dependency/derivative patch, so public GitHub, Play, or beta distribution is blocked until permission is recorded or the dependency is replaced. Ownership, rebase, regression, and rollback policy is documented in [`RNTP_PATCH_OWNERSHIP.md`](./RNTP_PATCH_OWNERSHIP.md).

- If iOS is a real near-term target, keep React Native and turn the patched player into a maintained internal fork or owned native modules.
...
| Dimension | Production web | Android working tree verified through 2026-07-16 | Remaining gap |
|---|---|---|---|
...
| Playback | Browser player, two-level product queue, retry/skip recovery | Native Media3/RNTP, connected-readiness gate, bounded retry/backoff, position restore, safe stop/skip, stable semantic queue identities, manual-before-context insertion, context-only shuffle/restore, clear-upcoming, encrypted paused process restoration, explicit history/current/manual/context presentation, and one versioned queue fixture consumed by both engines | Repeat/radio/background/Android Auto/reboot lifecycle coverage and rebuilt on-device evidence for the new queue presentation remain open. |
...
| Release evidence | Web behavior in production | Exact-current-source gates now record 934 mobile Vitest cases across 146 files, 10 web tests plus green web lint/build, 40/40 API tests with 274 subtests, 250 retained native release JVM tests, 20/20 Expo Doctor checks, clean native generation/compilation, and an optimized v10012 ARM64 QA assembly/lint. Earlier v10005 production-connected playback/queue/like, clean-install, and Keystore instrumentation evidence remains retained. | v10012 is installed, preserves successful signed-out cleanup-to-Login, closes v10011's specific 2400×1080 auth clipping defect, and fails closed at the expected production v2 compatibility gate. Airplane-mode/download lifecycle, account-switch/filesystem/notification/Android Auto cleanup, Share/system-volume, remote exact-commit CI, licensed public redistribution, production signing/AAB, and the full lifecycle/network/accessibility matrix remain open. |
...
|---|---|---|
| TypeScript, lint, unit/model/data tests | **Passed on exact current source** | `npm run check` passed Android brand determinism, TypeScript, repository-wide ESLint, and 146 Vitest files / 934 tests. The two credential-blind Android harness suites retain 34/34 evidence; web passed 10/10 tests, lint, and a production Next build; API OpenAPI v2/Android contract drift checks plus `pytest` passed 40/40 tests and 274 subtests; Expo Doctor passed 20/20. |
...
| Native player unit tests | **Passed** | The complete current-hash RNTP release JVM run passed 250 tests across 19 suites with zero failures/errors/skips, including controller trust, numeric-string persistence, account-boundary cleanup, automatic-cache eviction, 34 sleep-timer integration tests, and three timer-state tests. This is JVM evidence, not instrumentation/device evidence. |
| Generated Android native extensions | **Passed generation, equality, compilation, and assembly** | A clean Expo prebuild generated Share, media-stream binding, the explicit-download adapter, and Material Design Icons autolinking. Offline plugin/source contracts retain 6/6 evidence; normalized plugin-template and generated Kotlin output are identical, including the corrected explicit-store path validation; final Kotlin compilation and v10012 release assembly passed. The APK contains `assets/fonts/MaterialDesignIcons.ttf` (`1,307,660` bytes). This does not claim Share, volume, the full hostile-filesystem matrix, or offline runtime acceptance. |
| Native device instrumentation | **Partially passed** | `:rntp_player:connectedDebugAndroidTest` on the API 36 emulator; 2/2 passed: `BrowseTreeStorageInstrumentedTest` and `PlayerStateStorageInstrumentedTest`. They cover Android Keystore AES-GCM browse/auth restoration and encrypted native queue/product-metadata restoration, but not the exported service, external logout-deletion evidence, key invalidation, reboot, or a hostile cross-UID controller. |
| Android lint | **Passed on exact current source with 29 warnings** | v10012 `:app:lintRelease` has zero errors and 29 warnings: exported playback service, unverified App Link, ten intent-filter unique-data warnings, six private-resource warnings, two scoped-storage warnings, one discouraged API, one unused attribute, and seven unused resources. Launcher encoding/full-square regressions are absent. The exported-service and App Link security warnings remain open release work. |
| Release assembly | **Passed for exact-current-source v10012 local QA milestone; installed for auth-layout smoke** | With `ANDROID_VERSION_CODE=10012`, production API base, explicit QA debug-signing opt-in, ARM64-only architecture, R8 minification, and resource shrinking, the 538-task build passed Hermes bundling, `:app:lintRelease`, and `:app:assembleRelease` in `1m02s`. The APK is ARM64-only, min API 24/target API 36, contains Hermes bytecode plus the canonical production origin and `/api/version`, is v2/debug-signed, and is 16 KiB aligned. It clean-installed on API 36 and cold-started in `150 ms` on a stable PID without a fatal exception. Production still returns HTTP 404 from `/api/version`, so the compatibility gate intentionally stops before authentication/API/media access. |
| Standalone clean-install startup | **Passed for v10005** | On isolated API 36 ARM64 `emulator-5556`, the harness uninstalled/reinstalled the non-debuggable v10005 APK, verified cold and warm embedded-bundle starts on stable PID `4383`, found no Metro reverse/runtime dependency or app-scoped crash/ANR/native/ReactNativeJS failure, and observed the exact runtime production-origin marker. Authentication/navigation were deliberately outside `--startup-only`. |
...
| Candidate APK | **Exact-current-source local QA milestone; installed, not published** | `mobile/android/app/build/outputs/apk/release/app-release.apk`, mirrored as `/Users/logge/Documents/Codex/2026-07-15/c/outputs/LoggeRythm-1.0.1-10012-arm64-QA-debug-signed.apk`; `29,099,551` bytes, ARM64, min API 24, target API 36, version `1.0.1`/`10012`, SHA-256 `2176e4a2450c3f0ca1696135156e5534ee5a89509f60204f3d049f46ca544141`. It is v2/debug-signed with certificate SHA-256 `fac61745dc0903786fb9ede62a962b399f7348f0bb6f899b8332667591033b9c`, 16 KiB zip-aligned, and optimized. Production/signing/licensing gates mean it must not be represented as production-signed or production-compatible. |
| Candidate diagnostics | **Retained locally; not published** | Matching v10012 R8 mapping (`37,395,228` bytes; SHA-256 `58c696906c6093e212cc7f8a518fa48ac1adadd97f3e80a7dae0898ccdc4aef1`), R8 resources (`2,125,515` bytes; SHA-256 `5ec158f6e3545dd38bc8f9caaef8ef07d74ff1194867c10383b59ce049c62470`), native debug symbols (`5,095,603` bytes; SHA-256 `feaae76fae924084ec8a6099c57c68312bf830f74e36441b26ef5e78dde1861f`), Hermes source map (`7,206,009` bytes; SHA-256 `6cbe3f183605ebde648b67a6ea6fda5f16748a78afd378b1b73cfeeb9ffbc06a`), lint HTML (`186,057` bytes; SHA-256 `ebe4bbfe804481bc4d1ec2859f814dd14b42c74a6739cedd266d43c5da8f3899`), and lint XML (`48,993` bytes; SHA-256 `45eac78d00be6f988774c3b8645eadad6ad21349325f97da6d992fcb844e9edd`) are mirrored beside the APK with a complete artifact checksum manifest. They are not tied to any commit/tag or remote workflow run. |
| Remote release evidence | **Not run for current source** | Read-only checks confirm repository `LoggeL/LoggeRythm`, default branch `master`, healthy Keychain-backed CLI auth, and latest `Native Android QA` success run `29440802796` for base `e2ecf76` on 2026-07-15. Existing final release `v1.0.1` is occupied; older native QA prereleases also exist. v10012 comes from the large uncommitted working tree at that same base and therefore has no exact-source workflow, commit, tag, or GitHub Release. Authentication and prior releases do not satisfy current release gates. |

### 4.2 Highest-risk open gates

- **Architecture:** migrate the remaining hand-written runtime wrappers and feature caches through the generated-operation adapter into explicit domain models, type the eight underspecified backend success families, add cross-client production fixtures, roll out the already-implemented v2 compatibility endpoint/gate, and either record RNTP redistribution permission or replace it. The first descriptor-driven Album Search seam and duplicate-safe playlist-entry v2 seam pass locally, but production currently returns 404 from `/api/version`.
- **Queue:** repeat/radio/background/Android Auto/reboot lifecycle coverage and rebuilt emulator verification of the new history/current/manual/context sections. The versioned web/Android golden contract, semantic caller contexts, labels, announcements, and encrypted paused process restoration now pass locally.
- **Auth/security:** deterministic source tests cover first-admin/invite/pending/recheck/restart/Retry/Forget/401/403 behavior, CAS session revisions, serialized storage cleanup, and a fail-closed cleanup barrier. v10009 exposed a false `storage-scope-invalid` result when an explicit-store child did not yet exist; v10011 rebased children on the validated canonical `no_backup` boundary and combined lexical containment with double-`lstat` missing-child validation. v10012 retains successful signed-out cleanup to Login and logs only allowlisted boundary/code identifiers. Successful account-switch plus filesystem/queue/cache/notification/Auto evidence, hostile cross-UID controller instrumentation, secret-leak evidence, and credential rotation/history remediation remain open.
...
- **Platform/release:** verified App Links, Auto pagination/partial refresh/mutation refresh, real lifecycle/network-fault automation, exported-service hostile-client coverage, permitted dependency distribution, production signing/AAB, upgrade/rollback, symbol publication, dependency-advisory disposition, and exact-source CI/version/tag/release evidence. GitHub CLI authentication and the existing `v1.0.1` tag collision are already resolved facts, not open discovery work.
...
**Production rollout blocker:** the latest read-only check found that `https://loggerythm.logge.top/api/version` still returns HTTP 404 with `{"detail":"Not Found"}`. The Android v2 compatibility gate intentionally treats that response as definitively unsupported and caches the failure for the process before authenticated/API/media access. Installed v10012 preserves that fail-closed behavior. No production deployment was performed and no credential was entered or sent. Required order: deploy the backend endpoint, verify a public HTTP 200 response advertising compatibility with `v2`, record the exact backend revision, then rerun Android/emulator evidence. Do not weaken HTTP 404 into silent compatibility.

The repository has no production deployment workflow. Its Dokploy-oriented image bundles FastAPI and Next.js in one container, while the tracked branch, auto-deploy trigger, environment, `/data` mount, and rollback target live outside this repository. The compatibility router/schema/version files are currently untracked and the base `e2ecf76` does not contain them; the dirty backend also includes broader auth/contract changes. Do not push the whole working tree as an endpoint hotfix. Before any push, inspect Dokploy to learn whether the tracked branch auto-deploys, preserve the exact production environment and persistent volume, prepare and test a clean reviewed candidate SHA, record the last-known-good image/SHA and database backup, then obtain explicit production-change approval for the combined API/web restart. Roll back by redeploying that recorded image with the same environment and `/data`; a restored 404 intentionally leaves current Android fail-closed.
...
DATA-07 is closed at the source/test boundary. Typed feedback mappers now cover auth/bootstrap/logout/player startup, Profile save/delete/timer, catalog playback/follow/local storage, Library and playlist actions, global track actions, Spotify import, and the shared player error banner. Arbitrary URLs, response bodies, emails, tokens, storage keys, native codec details, and cleanup diagnostics collapse to localized action copy. Deliberately authored server-compatibility recovery, local registration/profile validation, sleep-timer outcomes, and player-recovery explanations remain specific through explicit typed markers. The player banner no longer writes raw native/transport detail to Logcat. Six pure mapper suites pass 12 privacy tests, the affected component/lifecycle suites assert alert/live behavior, and the complete 146-file / 930-test mobile gate is green. Rebuilt-APK TalkBack and log inspection remain under QA-15/18 and SEC-04 rather than reopening the satisfied presentation boundary.
...
DATA-06 is closed in current source. Media-item transition side effects use `Promise.allSettled`: play-history or radio-extension failure cannot pause, skip, reject the foreground/headless listener, or mutate the queue. Malformed transition metadata is contained the same way. Successful play-history persistence invalidates only account stats; a failed write does not. These events use a separate eight-second, deduplicated, dismissible notice store with localized generic copy; a fatal audio error suppresses the notice rather than being masked by it. Android Auto browse refresh failure now uses that channel too and never forwards raw backend/native diagnostics into the UI. Store, recovery, like/Auto, controller-contract, and full mobile tests cover TTL/replacement/dismissal, safe copy, unchanged audio/queue state, and success-only stats invalidation.
...
PLAYER-04 is implementation-complete but deliberately remains unchecked until rebuilt visual evidence satisfies its recorded acceptance boundary. The title opens only a validated album route; every credited artist is rendered in server order as its own validated ≥48 dp native link, while malformed legacy references remain readable and inert. The selected credit—not merely the primary artist—is forwarded to navigation. The production web cover helper is mirrored exactly for Deezer size segments; Playing uses viewport-bounded, vertically scrollable framed 1000 px artwork or a branded equalizer placeholder, and every tab sits over a separately bounded 480 px static, pointer-free, accessibility-hidden blurred-cover backdrop. Embedded Queue is transparent only in this owner so the ambient layer remains visible without forking queue behavior. Dedicated cover/metadata/layout/navigation suites pass five files / 18 cases; the integrated artwork/metadata/layout/navigation/queue slice passes six files / 23 cases; and the full mobile gate passes 146 files / 934 tests with TypeScript and ESLint clean. Dynamic palette extraction and the visualizer remain separately open under PLAYER-07/08. Installed v10012 proves native font/branding and signed-out Login/Register portrait/short-landscape layouts but cannot reach authenticated playback while production lacks compatible `/api/version` metadata, so PLAYER-04 still requires the screenshot/layout pass under QA-11/15.
...
Evidence: every new semantic queue context requires and persists a human-readable label, while queues restored from a pre-label APK use a localized type fallback. A pure presentation contract partitions canonical native indices into separately headed history, current, manual-upcoming, and labeled context-upcoming sections. The header and clear outcome count only upcoming items; mutation calls and stable test IDs continue to use native queue indices/identities. The UI supports jump, stable-ID remove, labeled move-up/down reorder, and clear-upcoming with exact accessibility outcomes; controller tests cover active/no-op/bounds/current protection, repeated IDs, stale rendered queues, and clear semantics. Every queue row includes duration, hydrated explicit-download state, and positive server-cache membership; only the active row may show positive RNTP rolling device-cache seconds. Downloaded, server cache, and rolling LRU are separate facts, and unknown manifest state is not rendered as “not downloaded.” Production v10005 verified accessible reorder and clear-upcoming but predates the download marker. Deterministic metadata/presentation tests cover retained history, idle queues, unknown/corrupt duration/cache values, explicit-download tri-state, legacy/null metadata, conflicting contexts, and context labels. Rebuilt emulator visual/TalkBack evidence remains tracked under QA-15 rather than reopening the satisfied UI contracts.

#### Playback settings and accounting

- [x] **PLAYER-13 (P1)** Add sleep timer presets 15/30/45/60 minutes and end-of-track; expose remaining/cancel state and preserve native background behavior.
- [ ] **PLAYER-14 (P2)** Implement real 0–12 second crossfade in the native Media3 seam, with gapless/background/queue tests; do not expose a non-functional setting.
...
Evidence for PLAYER-13: Profile exposes 15/30/45/60-minute and end-of-track choices plus remaining and cancel state through the native player command surface. The MediaLibraryService owns the timer rather than a foreground JavaScript timeout. The corrected native controller keeps the end-of-track timer armed across manual Next/Previous and index shifts, fires only for natural auto/repeat completion (including repeat-one) or final `STATE_ENDED`, and returns deterministic optimistic command state instead of an immediate stale storage read. Twenty-one JavaScript tests and 37 focused native JVM tests passed for the latest behavior. The installed v10005 APK proved only starting/cancelling a 15-minute timer; manual-skip, repeat-one, final-item, and locked/background behavior still require rebuilt device evidence under QA-10 rather than reopening this implemented product contract.
...
- [ ] **SEC-06 (P0)** Run the existing Keystore instrumentation coverage in CI rather than merely compiling it.
- [ ] **SEC-07 (P0)** Verify untrusted external media controllers are rejected and receive no privileged commands/metadata.
...
Progress: [`ANDROID_SESSION_THREAT_MODEL.md`](./ANDROID_SESSION_THREAT_MODEL.md) documents assets, trust boundaries, token flow, threats/controls/residual risk, invariants, accepted React Native bridge risk, native-ownership target, and mandatory production conditions, closing SEC-01. Production builds hard-fail a non-canonical API-origin override, config errors redact the supplied origin, runtime logs expose only the exact sanitized origin marker, and working-tree scans find no known credential fragments. Both existing Keystore instrumentation tests pass locally, while four controller-policy unit tests reject untrusted/non-privileged callers. Logout and defensive account switch now use a compare-and-swap session revision, serialize offline identity/recent-search writes behind cleanup, invalidate the explicit-download registry before awaiting native cleanup, and synchronously unmount account-owned UI. The native store supports exact-scope and unknown-account all-scopes erasure; both cancel active work, advance cleanup generations before deletion, reject new work while clearing, and verify controlled-root removal. Runtime tests prove queued/late operations cannot republish stale file URIs, mismatched concurrent clears promote to global clear, and a cleanup failure retains the auth barrier. The manifest is AES-GCM/Keystore encrypted and scope-bound; MP3 files are app-private/no-backup but not encrypted, so the document does not overstate at-rest protection. v10009 exposed a genuine fail-closed integration defect: canonicalizing an absent explicit-store child produced a false `storage-scope-invalid` result. The corrected native path validator first validates the canonical `no_backup` boundary, rebases children onto it, then requires lexical containment and two `lstat` checks for missing-child creation; only allowlisted cleanup boundary/code values are logged. The plugin and generated Kotlin normalize identically, focused regressions plus the full 934-test mobile gate pass, and installed v10012 retains successful signed-out cleanup to Login without cleanup warnings or fatal errors. SEC-05 therefore now requires filesystem/UI inspection of explicit downloads, rolling cache, notification, errors, Query state, and Auto tree across authenticated logout/account switch rather than diagnosis of the fixed signed-out path. SEC-06 remains open until CI executes instrumentation, including explicit-store cleanup coverage. SEC-07 remains open until a separate-UID client actually binds/queries the exported Media3 service. SEC-09 requires external credential rotation and an explicit history-remediation decision; no history rewrite was attempted.
...
- [ ] **QA-04 (P0)** Run `connectedAndroidTest`, including Keystore and controller-trust tests, on CI emulator hardware.
...
QA-01 is configured and regression-guarded locally but not remotely proven: push and pull-request filters include `api/**`; the Android workflow checks OpenAPI/generated Android drift before mobile/build gates, and a dedicated API-contract workflow checks both artifacts for any API or generated-contract change. Static tests assert those triggers and commands. Close it only after an API-only commit/PR actually triggers and passes those jobs. QA-02 remains partial: complete v2 generated wire/operation coverage, exact-status request-adapter coverage, the Album Search wire→domain seam, duplicate-safe playlist entry mutations, repository wiring, likes-query invalidation, queue golden cases, and radio deduplication/stale races have coverage, but full feature-domain mapping and an explicit play-count transition-idempotency test are absent.
...
Current evidence remains local: historical v10005 production playback/queue/like and Keystore evidence are retained; current v10012 adds exact APK/diagnostic hashes, clean generation with normalized plugin/generated Kotlin equality, an optimized/minified/shrunk 538-task ARM64 build/lint in `1m02s`, API 36 clean install, `150 ms` cold start on a stable PID, successful signed-out cleanup to German Login, and portrait Login/Register visual/accessibility checks. At 2400×1080, both bounded-width cards scroll to fully visible submit/mode-toggle controls above the gesture bar, and Login IME focus remains visible; this closes only v10011's specific auth clipping defect. Evidence is retained as `loggerythm-b10012-login`, `register`, `login-landscape`, `login-landscape-bottom`, `login-landscape-keyboard`, `register-landscape-initial`, and `register-landscape-bottom` PNG/XML pairs. No credential was entered. QA-04 is not complete because instrumentation has not run in remote CI and controller trust still needs a real cross-UID test. QA-06 remains open specifically for authenticated logout/account-switch cleanup. QA-07/08 remain partial because the production compatibility gate correctly prevents current-source authenticated playback and the lifecycle/repeat/radio/background/Auto/reboot matrices are incomplete. QA-18 remains partial: no exact commit/workflow/backend revision or explicit manual-exceptions manifest exists, and the local artifact set is not tied to a tag or remote run.
...
- [ ] **REL-03 (P0)** Define ABI policy. Keep optimized ARM64 production output and produce x86_64/universal QA output when emulator support is a release requirement.
- [ ] **REL-04 (P0)** Retain and actually upload R8 mapping/resources/native-debug-symbol artifacts, and associate them with the exact release tag/version.
- [ ] **REL-05 (P0)** Use monotonic `versionCode`, semantic version name, reproducible dependency lock, and provenance/checksum.
...
- [ ] **REL-08 (P0)** Publish only after every required P0/P1 gate is green; attach release notes, APK/AAB, checksums, mappings/symbols policy, known deferrals, and evidence link.
- [ ] **REL-09 (P0)** Review and either remediate or formally waive the 11 moderate Expo/vector-icons/Xcode build-time npm advisory paths without applying the unsafe `npm audit fix --force` Expo downgrade.

Current release state: v10005 remains the last artifact with authenticated production playback evidence. Exact current source has a locally verified and installed v10012 optimized ARM64 QA milestone; the v10009 signed-out cleanup defect remains fixed and v10011's specific 2400×1080 auth clipping defect is closed, but production still returns HTTP 404 from `/api/version`. Version/tag `v1.0.1` is already occupied by base `e2ecf76`; no production keystore is configured, v10012 is debug-signed, no AAB exists, and the dirty working tree has not run remotely on an exact commit. The installed RNTP license and repository records do not establish redistribution permission, independently blocking GitHub draft/public releases, GitHub asset upload, Play/beta artifact upload, and source tagging. Matching v10012 APK, checksum, R8 mapping/resources, native symbols, Hermes map, lint reports, and UI evidence are retained locally only and have not been uploaded to GitHub. Switching to a permanent production certificate will require existing debug-signed installs to uninstall and lose app-local state. The production test credential previously committed to history must still be rotated/revoked and its history disposition decided. Do not publish a source tag or any GitHub Release until backend v2 compatibility, full account-switch cleanup evidence, permitted dependency distribution/replacement, exact-commit CI, REL-01…09, and required parity gates are satisfied.

Smallest safe publication sequence: complete and device-prove authenticated logout/account-switch cleanup; deploy `GET /api/version` and verify a public compatible-v2 HTTP 200 response tied to an exact backend revision; rotate/revoke the exposed test credential and sessions; record written RNTP redistribution permission or replace the dependency; intentionally commit the parity work; run the updated workflow on that exact commit; choose a non-colliding milestone tag such as `android-qa-YYYYMMDD.N-b10012`; rebuild from that exact commit with a monotonic version code and asserted certificate; retain R8/native/Hermes symbols; upload a version/code/ABI-qualified APK plus `.sha256`; then publish only within the permitted distribution channel. Never replace the existing `v1.0.1` asset with a build from different source.

## 6. API coverage appendix

The backend already supports almost all product areas; parity is primarily Android client work.

### Android wrappers present (71)
...
- Migrate the 71 hand-written runtime feature wrappers and the web consumer to the generated versioned operations instead of maintaining parallel runtime request/response shapes.
...
- Prove the existing route/schema regeneration and drift workflows on the exact candidate commit in remote CI, and model the eight backend success families that remain underspecified upstream.
...
- [x] Unsupported `MEDIA_PLAY_FROM_SEARCH` advertising was removed; release lint has zero errors (54 warnings remain tracked below).
...
- [ ] CI now configures native instrumentation, emulator, x86_64 QA, R8 and checksum steps, but this working tree has not executed remotely and native symbols are not yet proven uploaded.
...
- [ ] Production currently returns HTTP 404 from `/api/version`; deploy metadata advertising Android contract v2 before authenticated current-source testing or release.
...
- [ ] Release lint reports an exported playback service without a manifest permission. Runtime trust filtering is unit-tested, but no hostile cross-UID service/controller instrumentation exists.
...
- [ ] `npm audit --omit=dev` reports 11 moderate Expo/vector-icons/Xcode build-time advisory paths and zero high/critical findings. The suggested force fix would downgrade Expo and is not acceptable; review/waiver or a safe upstream resolution is required.
- [ ] The exact-current-source v10012 APK is ARM64/debug-signed, locally verified, and installed for a fail-closed compatibility plus cleanup/auth-layout smoke. Production v2 metadata, authenticated account-switch cleanup, licensing, production signing/AAB, exact-commit CI, and remaining parity gates still forbid a public GitHub/production release claim.
...
- Generated OpenAPI wire/operation contract: [`api/app/android_contract.py`](../api/app/android_contract.py), [`api/openapi/v1.json`](../api/openapi/v1.json), [`mobile/src/api/generated/contract.ts`](../mobile/src/api/generated/contract.ts)
...
- Player/queue/recovery/radio: [`mobile/src/player/controller.ts`](../mobile/src/player/controller.ts), [`mobile/src/player/queueContract.ts`](../mobile/src/player/queueContract.ts), [`mobile/src/player/recoveryPolicy.ts`](../mobile/src/player/recoveryPolicy.ts), [`mobile/src/player/setup.ts`](../mobile/src/player/setup.ts)
...
- Android Auto: [`mobile/src/player/browseTree.ts`](../mobile/src/player/browseTree.ts), [`mobile/plugins/withAndroidAuto.js`](../mobile/plugins/withAndroidAuto.js)
...
- RNTP ownership/licensing/rebase/rollback: [`docs/RNTP_PATCH_OWNERSHIP.md`](./RNTP_PATCH_OWNERSHIP.md)
