...
**Acceptance snapshot:** 28/62 P0 and 76/124 P1 work items are closed at this evidence revision; 34 P0 and 48 P1 items remain open. That is 104/186 closed (56% rounded). PLAYER-13 remains open until the implemented first-party sleep engine passes its full authenticated/background device matrix.

**Baseline:** 2026-07-15; evidence updated through 2026-07-16

**Source revision:** local integration checkpoint `ce78af4`; the identical public clean-root tree `9793c25` is tagged at commit `3d134b3` as `android-media3-v1.0.2-rc.1`

**Production reference:** `https://loggerythm.logge.top`

**Android reference:** production-connected `1.0.1` (`versionCode 10005`) remains the last APK with authenticated production playback evidence. The ARM64 `1.0.1`/`10012` RNTP APK remains a historical local-only checkpoint and must not be published. The replacement first-party Media3 `1.0.2`/`10013` QA APK passes clean-install cold/warm startup on the API 36 emulator, but is debug-certificate signed and not a production artifact. Production's HTTP 404 at `/api/version` correctly blocks authentication/API/media access, so account-switch/filesystem/notification/Android Auto cleanup and the broader authenticated device matrices remain release gates.
...
implemented. Offline `npm ci`, TypeScript, ESLint, and 149 Vitest files / 1,003
tests plus one explicit todo pass at the final checkpoint. The first-party
...
focused tests; the complete first-party JVM suite passes 59/59 and API 36
Keystore/persistence instrumentation passes 5/5. Session binding, atomic
...
Complete dead-React headless delivery, cold service-only Auto restore,
authenticated lifecycle proof, and production signing remain open. The final
gate scans 426 source/generated files and 1,090 APK entries with zero findings,
...
| Playback | Browser player, two-level product queue, retry/skip recovery | The product controller/queue behavior and 28 consumers are migrated to an owned facade; a first-party Media3 service, strict native mapper, trusted-controller policy, cache/preload, and progress ticker exist. Historical RNTP queue/restore evidence is retained only as a regression reference | Complete commands, sleep, encrypted lifecycle restore, headless/background/notification/Android Auto/reboot behavior, clean assembly, and rebuilt on-device proof remain open. |
...
| Release evidence | Web behavior in production | The final candidate passes offline `npm ci`, TypeScript, ESLint, 149 mobile Vitest files / 1,003 tests plus one todo, 59/59 first-party JVM tests, 5/5 API 36 instrumentation tests, release lint/assembly, the 426-source/1,090-APK zero-finding gate, and clean-install cold/warm emulator startup. Historical b10012 and v10005 evidence remains regression input only | Authenticated account cleanup, production v2, exact-commit remote CI, credential remediation, production signing/AAB, and the full lifecycle/network/accessibility matrix remain open. |
...
|---|---|---|
| TypeScript, lint, unit/model/data tests | **Passed for the final candidate tree** | Offline `npm ci`, TypeScript, repository-wide ESLint, and 149 Vitest files / 1,003 tests plus one explicit todo pass after the completed consumer cutover. Historical credential-blind harness, web, API, and Expo Doctor evidence is retained separately. |
...
| Native player unit tests | **Passed for the first-party candidate** | The first-party Media3 protocol/controller/cache/persistence/sleep/browse/policy suite passes 59/59 JVM tests. Complete authenticated lifecycle, dead-React product effects, cold Auto, and hostile separate-UID device matrices remain open. The historical RNTP 250-test result is regression input only and does not validate the replacement. |
| Generated Android native extensions | **Clean first-party regeneration, compile, and artifact proof passed** | Clean Expo prebuild regenerated the manifest/Gradle tree. The executable gate scans 426 source/generated files and 1,090 APK entries with zero findings, and Gradle lists `:loggerythm_player-native` with no old player project. Hermes, DEX/native payload, manifest, service actions, and dependencies were inspected. |
| Native device instrumentation | **5/5 first-party tests passed on API 36** | First-party Android Keystore/encrypted-persistence round trips pass on the ARM64 emulator after the empty-timeline serializer fix. Process death/reboot, notification/media buttons, authenticated account cleanup, key invalidation, and hostile separate-UID control remain open. |
| Android lint | **Passed on the first-party release candidate** | Release lint passes after the config plugin writes the fully qualified MediaLibraryService and both Media3/platform browser actions. Remaining exported-service security acceptance is tracked by the hostile-controller matrix rather than hidden as a lint exception. |
| Release assembly | **Passed for ARM64 Media3 `1.0.2`/`10013`** | Native release JVM tests, app release unit tests, lint, R8/minification, resource shrinking, and assembly pass with Hermes. The unpacked gate, v2 signature, 16 KiB alignment, nondebuggable mode, production-origin/no-Metro scan, manifest, Android Auto, and branding checks pass. The QA APK is debug-certificate signed; production signing/AAB and exact remote CI remain open. |
| Standalone clean-install startup | **Passed for Media3 v10013** | On the Android Studio API 36 ARM64 emulator, the harness uninstalled/reinstalled the nondebuggable v10013 APK and verified cold and warm embedded-Hermes starts, branded German Login, the exact production origin, no Metro reverse/runtime dependency, and no app-scoped crash/ANR/native/ReactNativeJS failure. Authentication was deliberately not attempted because `/api/version` returned 404 before credential submission. Historical v10005 proof remains regression input. |
...
| Candidate APK | **First-party Media3 QA candidate passed local gates** | `LoggeRythm-1.0.2-10013-arm64-Media3-QA-debug-signed.apk` is 27,363,030 bytes with SHA-256 `2bd490e96b0e0f58c42f8d300f662ad48ae2838002c5c623975638f6315d9378`. It is nondebuggable, ARM64-only, Hermes/R8 built, aligned and v2-signed, but uses the Android debug certificate and is strictly a QA/prerelease artifact. Historical b10012 remains local-only and must never be uploaded. |
| Candidate diagnostics | **Generated for v10013** | Matching R8 `mapping.txt` (33,070,116 bytes), `resources.txt` (1,876,754 bytes), checksum, clean-install evidence, and startup screenshot are frozen beside the candidate. Production symbols/signing and remote CI provenance remain open. |
| Remote release evidence | **QA prerelease published and asset digest verified** | GitHub prerelease [`android-media3-v1.0.2-rc.1`](https://github.com/LoggeL/LoggeRythm/releases/tag/android-media3-v1.0.2-rc.1) is published from clean-root commit `3d134b3`; both the release branch and dereferenced annotated tag resolve to that commit. GitHub reports the uploaded APK as 27,363,030 bytes with digest `sha256:2bd490e96b0e0f58c42f8d300f662ad48ae2838002c5c623975638f6315d9378`, matching the local candidate. The release is correctly marked prerelease and discloses debug signing plus the production compatibility 404. Exact-source remote CI and production/latest promotion remain open. |

### 4.2 Highest-risk open gates

- **Architecture:** finish the first-party player lifecycle and newly assembled APK gate; migrate the remaining hand-written runtime wrappers and feature caches through the generated-operation adapter into explicit domain models; type the eight underspecified backend success families; add cross-client production fixtures; and atomically deploy the already-implemented v2 compatibility/playlist contract under an identified authority. Clean prebuild/source gate and the Album Search/duplicate-safe playlist-entry seams pass locally, but production currently returns 404 from `/api/version`.
...
- **Platform/release:** newly assembled first-party APK gate; sleep/commands/headless/Auto cold-tree completion; verified App Links; real lifecycle/network-fault automation; exported-service hostile-client coverage; production signing/AAB; upgrade/rollback; symbol publication; dependency-advisory disposition; credential rotation/history decision; and exact-source CI/version/tag/release evidence. Clean prebuild and the zero-finding source/generated gate already pass; GitHub CLI authentication and the existing `v1.0.1` tag collision are resolved facts, not open discovery work.
...
PLAYLIST-05 is closed at the contract/source/test boundary. The existing database `PlaylistTrack.id` is now exposed as required positive `playlist_entry_id` on each playlist occurrence. Duplicate-safe v2 mutations delete `/api/playlists/{playlist_id}/tracks/entries/{entry_id}` and atomically patch `/api/playlists/{playlist_id}/tracks/entries/order` with the complete ordered `entry_ids` snapshot; duplicate, missing, foreign, or stale entry snapshots reject without partial position changes. Legacy Deezer-ID routes remain unchanged for v1 clients. Android decoders, repository mutations, optimistic caches, row ownership, and offline snapshots use entry identity while preserving the currently playing native queue snapshot, so removing one duplicate occurrence does not remove its siblings or rewrite an already-playing context. API coverage passes 40/40 tests with 274 subtests, and the final Android contract/mobile gate passes at OpenAPI v2 and 149 files / 1,003 tests plus one todo. Production device mutation evidence remains impossible until `/api/version` advertises v2, but that deployment gate is tracked under ARCH-03/QA rather than reopening the satisfied duplicate-safe product contract.
...
Historical evidence for PLAYER-13: the RNTP checkpoint exposed 15/30/45/60-minute and end-of-track choices plus remaining/cancel state and passed its focused JavaScript/native timer tests; v10005 proved only starting/cancelling a 15-minute timer. The first-party port retains the public shape, but its native sleep commands/engine are still pending. PLAYER-13 is therefore reopened until the first-party service owns the timer, passes repeat/manual-skip/final-item tests, and is proven locked/background on a rebuilt device under QA-10.
...
The first-party player now keeps credentials out of its Native-v1 snapshots and public Media3 metadata, enforces strict Cookie-vault/DataSource boundaries, rejects untrusted/non-privileged controllers in policy tests, and has a versioned encrypted codec/store with focused JVM plus 5/5 API 36 instrumentation coverage. Offline `npm ci`, TypeScript, ESLint, and 149 Vitest files / 1,003 tests plus one todo pass; the final gate scans 426 source/generated files and 1,090 APK entries with zero RNTP findings, and Gradle exposes only the first-party player project. Signed-out clean-start device proof exists, but this is not authenticated account-lifecycle acceptance. SEC-05 remains open until the single JS/Kotlin cleanup transaction and authenticated filesystem/queue/cache/notification/error/Query/Auto inspection pass. SEC-06 remains open until first-party instrumentation runs in CI; SEC-07 requires a real separate-UID controller; SEC-09 requires external credential rotation/revocation plus an explicit history decision.
...
Current first-party evidence includes offline `npm ci`, TypeScript, ESLint, 149 Vitest files / 1,003 tests plus one todo, 59/59 JVM tests, 5/5 API 36 instrumentation tests, clean Expo prebuild, release lint/assembly/R8, a 426-source/1,090-APK zero-finding gate, fresh-install cold/warm startup on the Android Studio emulator, and a digest-verified GitHub QA prerelease from exact clean-root commit/tag `3d134b3`. Historical v10005 production playback/queue/like evidence and b10012 auth-layout evidence remain regression references only; b10012 contains RNTP/derivative code and must not be published. QA-04 stays open because controller trust still needs real hostile cross-UID instrumentation and remote CI. QA-06 remains open for authenticated atomic logout/account-switch cleanup. QA-07/08 remain partial because production's compatibility 404 prevents current-source authenticated playback and lifecycle/repeat/radio/background/Auto/reboot matrices are incomplete. QA-18 remains partial until exact remote workflow/backend revision and the explicit manual-exceptions manifest are recorded.
...
Current release state: v10005 remains the last artifact with authenticated production playback evidence, while b10012 remains a historical local-only RNTP checkpoint. The current first-party tree has passed offline install/checks and clean prebuild/source-generated gates, but it has no assembled APK, emulator acceptance, exact commit, or remote CI evidence. Production still returns HTTP 404 from `/api/version`; backend v2 exists only locally and requires an atomic metadata/playlist-contract deployment under an identified authority. Version/tag `v1.0.1` is already occupied, no production keystore is configured, and no AAB exists. Removing RNTP from the clean source eliminates that dependency from a future artifact only after the unpacked-APK gate passes; it does not make b10012 or its lineage publishable. The previously exposed production test credential must still be rotated/revoked and its history disposition decided. Do not publish a source tag or GitHub Release until the first-party APK/runtime gates, backend v2, full account-switch cleanup, credential remediation, exact-commit CI, REL-01…09, and required parity gates are satisfied.

Smallest safe publication sequence: finish session binding, encrypted lifecycle restore, sleep/commands, headless/Auto behavior, and the atomic account-cleanup boundary; compile and device-prove them; assemble a first-party APK and pass the Hermes/DEX/native/unpacked-artifact gate; atomically deploy `GET /api/version` plus the v2 playlist contract and verify a public compatible-v2 HTTP 200 response tied to an exact backend revision; rotate/revoke the exposed credential and decide history remediation; commit a clean tree that has no b10012 derivative ancestry in the publication branch; run the release workflow on that exact commit; choose a new non-colliding version/tag; build with a monotonic version code and asserted certificate; retain R8/native/Hermes symbols; upload a version/code/ABI-qualified APK plus `.sha256`; then publish only within the approved channel. Never replace the existing `v1.0.1` asset or upload b10012.
...
- [ ] Production currently returns HTTP 404 from `/api/version`; read-only probes on 2026-07-16 confirm that public v1 routes still answer while playlist detail omits v2's unique `playlist_entry_id`. Deploy metadata plus the v2 playlist contract before authenticated current-source testing or release; do not weaken the client gate into an ambiguous duplicate-track fallback.
...
- [ ] Historical b10012 is ARM64/debug-signed, locally verified, and installed for a fail-closed compatibility plus cleanup/auth-layout smoke, but contains RNTP/derivative code and is not built from the current tree. The first-party source has no APK yet. Production v2 metadata, authenticated account-switch cleanup, credential remediation, production signing/AAB, exact-commit CI, and remaining parity gates still forbid a public GitHub/production release claim.
