...
export interface LoudnessTrackFields {
...
export function calculateLoudnessGain(
  ...
  // Current playback sinks expose bounded 0..1 output gain. Normalize by attenuating
  // loud tracks only: no digital boost, no clipping, same behavior on web and Android.
  const gainDb = clamp(
    rawGainDb,
...
export function loudnessMetadataFromTrack(track: LoudnessTrackFields): LoudnessMetadata {
  return {
